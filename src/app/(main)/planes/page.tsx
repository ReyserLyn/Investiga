...
import { getPlanesMembresias } from "@/lib/data/planesMembresia";
import { Check } from "lucide-react";
import Link from "next/link";

export default function PlanesPage() {
  const planesList = getPlanesMembresias();

  return (
    <section id="planes" className="container py-12 sm:py-24 flex-1">
        ...
        {planesList.map((pricing) => (
          ...
          >
            <CardHeader>
              ...
              <CardDescription className="min-h-[85px] xl:min-h-[70px]">
                {pricing.description}
              </CardDescription>
            </CardHeader>
              ...
              <div className="space-y-4">
                {pricing.benefitList.map((benefit: string) => (
                  <span key={benefit} className="flex">
                    <Check className="text-green-500 min-w-7" />{" "}
